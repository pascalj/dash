...
void sort(GlobRandomIt begin, GlobRandomIt end, SortableHash sortable_hash)
{
  ...
  for (auto const& unit : p_unit_info.valid_remote_partitions) {
    ...
    merge_dependencies.emplace(unit_range, std::async(std::launch::async, [&] { fut.wait(); }));
  }
  ...
  merge_dependencies[local_range] = std::async(
      std::launch::async,
      [send_count, send_disp, lcopy, target_disp, lbegin] {
        if (send_count) {
          std::copy(
  ...
  for (std::size_t d = 0; d < depth; ++d) {
    ...
    // Generate a merge tree that can be processed
    for (std::size_t m = 0; m < nmerges; ++m) {
      ...
      auto&& fut = std::async(
          [first, mid, last, f, mi, l, d, &merge_dependencies]() {
            ...
            DASH_LOG_TRACE("merged chunks", f, mi, l);
          });
